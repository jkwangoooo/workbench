...
test('stage1 page is isolated to 8班 and has four-field search plus masked profiles', async () => {
  const [page, domain] = await Promise.all([
    ...
    readFile(join(root, 'src/modules/class-management/domain/student.ts'), 'utf8')
  ]);
...
test('student ID profile field is grouped and masked by default', async () => {
  const domain = await readFile(join(root, 'src/modules/class-management/domain/student.ts'), 'utf8');
...
test('student repository keeps profile reads separate from roster list', async () => {
  const source = await readFile(join(root, 'src/modules/class-management/data/studentRepository.ts'), 'utf8');
...
test('migration has RLS and owner/class scope trigger', async () => {
  const sql = await readFile(join(root, 'supabase/migrations/202608310002_stage1_student_directory.sql'), 'utf8');
...
test('preflight rules preserve IDs and reject profile data for 7班', async () => {
  const source = await readFile(join(root, 'src/modules/class-management/imports/updatePreflight.ts'), 'utf8');
