...
test('stage2 migration keeps grouping and seating tables and RPCs independent', async () => {
  ...
  assert.match(sql, /class_group_layouts/); assert.match(sql, /class_seating_layouts/); assert.match(sql, /replace_group_layout/); assert.match(sql, /replace_seating_layout/); assert.match(sql, /enable row level security/); assert.match(sql, /student owner\/class mismatch/);
...
test('stage2 parsers enforce fixed template contracts and duplicate rejection', async () => {
  ...
  assert.match(group, /组别/); assert.match(group, /学生重复/); assert.match(group, /filter\(\(\{ name \}\)/); assert.match(seating, /模板版本/); assert.match(seating, /结构标记未知/); assert.match(page, /分组表/); assert.match(page, /座次表/); assert.match(page, /确认保存/); assert.match(page, /'EMPTY'/);
});
...
test('stage2 fixed seating structure and refresh mappings are explicit', async () => {
  ...
  assert.match(domain, /rowIndex === 0 \? 'podium'/); assert.match(domain, /columnIndex === 4 \? 'aisle'/); assert.match(domain, /座次表必须是8行9列/);
  assert.match(groupRepository, /template_version/); assert.match(groupRepository, /group_index/); assert.match(groupRepository, /displayName/); assert.match(groupRepository, /mapGroupLayout/);
  assert.match(seatingRepository, /row_index/); assert.match(seatingRepository, /column_index/); assert.match(seatingRepository, /cell_kind/); assert.match(seatingRepository, /mapSeatingLayout/);
});
...
test('stage2 upload previews are isolated from saved layouts and save only on confirmation', async () => {
  ...
  assert.match(page, /savedGroupLayout/); assert.match(page, /draftGroupLayout/);
  assert.match(page, /savedSeatingLayout/); assert.match(page, /draftSeatingLayout/);
  ...
  assert.match(confirmSection, /this\.groups\.replace/); assert.match(confirmSection, /this\.seating\.replace/);
});
...
test('stage2 verification SQL is metadata-only', async () => {
  ...
  assert.match(sql, /information_schema\.columns/); assert.match(sql, /pg_policies/); assert.match(sql, /pg_indexes/); assert.match(sql, /information_schema\.triggers/); assert.match(sql, /information_schema\.routines/);
